...
import 'package:weitong/pages/Admin/AdminSearch/admin_search_message_content_list.dart';
...
class AdminSearchConversationPage extends StatefulWidget {
  final Map arguments;
  AdminSearchConversationPage({Key key, this.arguments}) : super(key: key);

  @override
  State<StatefulWidget> createState() =>
      _AdminSearchConversationPageState(arguments: this.arguments);
}

class _AdminSearchConversationPageState
    extends State<AdminSearchConversationPage>
    implements MessageContentListDelegate {
  String pageName = "example.AdminSearchConversationPage";
  Map arguments;
  ...
  AdminSearchMessageContentList messageContentList;
  ...
  _AdminSearchConversationPageState({this.arguments});
  @override
  void initState() {
    super.initState();
    _requestPermissions();

    // messageContentList = arguments["conversation"];
    messageDataSource = arguments["conversation"];

    messageContentList = AdminSearchMessageContentList(
...
//conversationType==RCConversationType.Group?
  ...
  void dispose() {
    ...
    // EventBus.instance.commit(EventKeys.AdminSearchConversationPageDispose, null);
  ...
  _addIMHandler() {
    ...
    RongIMClient.onMessageDestructing =
      ...
      if (remainDuration == 0) {
        ...
        _refreshMessageContentListUI(); //融云调用刷新
      }
    };
