...
import 'package:weitong/pages/Admin/AdminSearch/admin_search_conversation_item.dart';
// import 'package:weitong/pages/SearchMessage/search_conversation_item.dart';
import 'package:weitong/services/ScreenAdapter.dart';
import 'package:weitong/services/event_bus.dart';
import 'package:weitong/widget/bloc/message_bloc.dart';
// import 'package:weitong/widget/conversation_item.dart';
import 'package:weitong/widget/widget_util.dart';

//传入targetId（对方的id），conversationType对话类型，可以获得历史消息并展现，并可以收到新的消息
class AdminSearchMessageContentList extends StatefulWidget {
  MessageContentListDelegate delegate;
  List messageDataSource = new List();
  bool multiSelect;
  List selectedMessageIds = new List();
  _AdminSearchMessageContentListState state;
  Map burnMsgMap = Map();
  AdminSearchMessageContentList(
  ...
  State<StatefulWidget> createState() {
    return state = _AdminSearchMessageContentListState(messageDataSource,
        multiSelect, selectedMessageIds, delegate, burnMsgMap);
  }
}

class _AdminSearchMessageContentListState
    extends State<AdminSearchMessageContentList>
  ...
  _AdminSearchMessageContentListState(
      List messageDataSource,
      bool multiSelect,
      List selectedMessageIds,
      MessageContentListDelegate delegate,
  ...
  void initState() {
    ...
    super.initState();
    updateData(messageDataSource, multiSelect, selectedMessageIds);
    EventBus.instance.addListener(EventKeys.BurnMessage, (map) {
      int messageId = map["messageId"];
      int remainDuration = map["remainDuration"];
      AdminSearchConversationItem item = conversationItems[messageId];
          ...
          return ListView.separated(
              ...
              itemBuilder: (BuildContext context, int index) {
                if (messageDataSource.length != null &&
                  ...
                  Message tempMessage =
                      MessageModelToConversation.conversatioToMessage(
                  ...
                  List<Message> messageDataSource2 = new List();
                  for (int i = 0; i < messageDataSource.length; i++) {
                    Message t = MessageModelToConversation.conversatioToMessage(
                        messageDataSource[index]);
                    messageDataSource2.add(t);
                  }
                  AdminSearchConversationItem item =
                      AdminSearchConversationItem(
                          this,
                          tempMessage,
                          // _needShowTime(index, messageDataSource),
                          _needShowTime(index, messageDataSource2),
                          this.multiSelect,
                          selectedMessageIds,
                          time);
  ...
  void _refrshItem(Message msg) {
    AdminSearchConversationItem item = conversationItems[msg.messageId];
    item?.refreshUI(msg);
  }
