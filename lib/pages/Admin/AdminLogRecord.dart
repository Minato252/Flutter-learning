...
class AdminLogRecordPage extends StatefulWidget {
  AdminLogRecordPage({Key key}) : super(key: key);

  @override
  _AdminLogRecordPageState createState() => _AdminLogRecordPageState();
}

// class _AdminLogRecordPageState extends State<AdminLogRecordPage>
...
class _AdminLogRecordPageState extends State<AdminLogRecordPage> {
  ...
  searchMessage() async {
    ...
    String useid = prefs.get("adminId");
    var type = await Dio()
  ...
  _awaitReturnChooseStaff(BuildContext context) async {
    ...
    var type = await Dio()
        .post("http://47.110.150.159:8080/gettype?id=" + adminId); //获取体系
    var allUsers = await Dio()
        .post("http://47.110.150.159:8080/selectMember?type=" + type.data);
    List users2 = allUsers.data;
    Map userOne = users2[0];
    userOne["uLoginid"];
    // String jsonTree = await Tree.getTreeFormSer(userInfo["id"], false, context);
    String jsonTree =
        await Tree.getTreeFromSer(userOne["uLoginid"], false, context);
